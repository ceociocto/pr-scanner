...
export class EvaluationRepository {
  ...
  insert(pullRequestId: number, scanId: string, result: EvaluationResult): void {
  ...
  getByScanId(scanId: string) {
    const db = getDb(this.config);
    return db.select().from(evaluations).where(eq(evaluations.scanId, scanId)).all();
  }

  /** Get evaluations for a specific PR */
  getByPullRequestId(pullRequestId: number) {
    const db = getDb(this.config);
    return db.select().from(evaluations).where(eq(evaluations.pullRequestId, pullRequestId)).all();
  }
